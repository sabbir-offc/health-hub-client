...
import { updateTestDetails } from "../../../../api/admin";
...
const UpdateTest = () => {
  ...
  const { title, details, price, slots, date, _id } = test;
  const [imgBtnText, setImgBtnText] = useState("Upload Test Thumbnail Image");
  const [selectedImg, setSelectedImg] = useState(undefined);
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  const [selectedDate, setSelected] = useState(new Date(date));
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const dbResponse = await updateTestDetails(_id, testInfo);
